...
The DataSHIELD Interface (DSI) defines a set of [S4 classes and generic methods](http://adv-r.had.co.nz/S4.html) that can be implemented for accessing a data repository supporting the DataSHIELD infrastructure: controlled R commands to be executed on the server side are guaranteeing that non disclosive information is returned to client side.
...
Then we use the DataSHIELD functions from the dsBaseClient package to analyse the data and get non-disclosive summary statistics. For example we can get the column names and the dimension of the dataset from each study, we can get the summary of a variable which includes its class, length, quantiles and mean, we can get contigency tables between categorical variables, we can apply a generalized linear model and many other functions:
