...
This is the how an Opal looks like once enter the credentials:
...
The tab `Projects` (top-left) goes to our available projects in the Opal demo
...
Then this window will appear and information of your resource must be filled in
...
The tab `Administration` (red circle in the previous figure) allows the user access to the administration part


![Administration tab at Opal](fig/opal_7.png)

The `DataSHIELD` tab goes to the DataSHIELD administration site

![Managing DataSHIELD packages in Opal](fig/opal_8.png)

The tab `+Add Pacakge` allow the user to install a DataSHIELD package either from DataSHIELD repository or any other GitHub site
...
All the dependencies in a DataSHIELD package are automatically installed when installing it into the Opal Server. If necessary the user can also use the `+Install` buttom from Administration/R tab
...
The user can also install R packages from CRAN/Bioconductor using the next R code:
